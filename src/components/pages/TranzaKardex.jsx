...
export default function TrazaKardez(props) {
    ...
    const selectitemData = (a) => {
        ...
        let tablat = document.getElementById('tablitas1')


      // Función para mostrar las propiedades correspondientes
function mostrarPropiedades(opcion) {
    if (opcion === '1') {
        ...
        btnt.style.display = 'none'; // Ocultar btnt en la opción 1
        tablat.style.display = 'none'; // Ocultar tablat en la opción 1
    } else if (opcion === '0') {
        ...
        btnt.style.display = 'none'; // Ocultar btnt en la opción 0
        tablat.style.display = 'none'; // Ocultar tablat en la opción 0
    } else if (opcion === '2') {
        emplo.style.display = 'none';
        tablae.style.display = 'none';
        tablas.style.display = 'none';
        ...
        pros.style.display = 'block';
        bode.style.display = 'block';
        btnt.style.display = 'block'; // Mostrar btnt en la opción 2
        tablat.style.display = 'block'; // Mostrar tablat en la opción 2
    }
}

// Llamada a la función cuando se selecciona una opción
mensaje = a.target.options[a.target.selectedIndex].text;
var opcionSeleccionada = a.target.options[a.target.selectedIndex].title;
mostrarPropiedades(opcionSeleccionada);
                ...
                success: function (json_data) {
                    ...
                    if (json_data !== 'Not Data') {
                        ...
                        for (let i = 0; i < json_data.length; i++) {
                            const option = document.createElement('option')
                            option.value = json_data[i]['id']
                            option.text = json_data[i]['detail_store'] + ' || Lote-' + json_data[i]['cod_lot'] + ' | ' + json_data[i]['cod_prod'] + ' | ' + json_data[i]['full_name'] + ' | Existencia ' + json_data[i]['total_quantity']
                            option.title = json_data[i]['cod_lot']
                            option.name = json_data[i]['cod_prod']
    ...
    const prueba = () => {
        ...
        }else if  (document.getElementById('opv').value === '2') {
            tbs = $("#tablitas1 tbody tr").length;
            value = 2
            if (tbs > 0) {
                $.ajax({
                    type: 'post',
    ...
    const AddDetailKardex = (id_header, valor) => {
        ...
        if (valor === 1) {
            ...
            for (let i = 1, row; row = document.getElementById('tablitas1').rows[i]; i++) {
                let quantity
                let quantityT
                quantity = -parseInt(parseInt(row.cells[5].innerHTML))
                quantityT = parseInt(parseInt(row.cells[5].innerHTML))
    ...
    const add_tablaT = () => {
        ...
        if (parseInt(formuk2.quantity.value) > parseInt(formuk2.cod_prods.options[formuk2.cod_prods.selectedIndex].role)) {
            ...
            if (formuk2.cod_prods.value !== '' && formuk2.quantity.value !== '') {
                ...
                console.log(formuk2.cod_prods.options[formuk2.cod_prods.selectedIndex].placeholder);
                var fila = "<tr><td>" + formuk2.cod_prods.value +  "</td><td>" + formuk2.cod_prods.options[formuk2.cod_prods.selectedIndex].name   + "</td><td>" + formuk2.cod_prods.options[formuk2.cod_prods.selectedIndex].placeholder + "</td><td className='celdas'>" + formuk2.cod_store.value  + "</td><td>" + formuk2.cod_prods.options[formuk2.cod_prods.selectedIndex].title + "</td><td>" + formuk2.quantity.value + "</td><td><button class='btn btn-danger  btn-sm' value='eliminar' title='Eliminar dato' onClick={this.closest('tr').remove()} type='button'><i class='fas fa-trash'></i></button></td></tr>"
                var btn = document.createElement("TR");
                btn.innerHTML = fila;
                document.getElementById("id_creacions1").appendChild(btn);
                                            ...
                                            <form id="formk2">
                                                ...
                                                <div class="card-body ">
                                                    ...
                                                    <div class="col-md-12">
                                                        ...
                                                        <button type="button" className="btn btn-info float-right mb-2" id="btnt" onClick={add_tablaT} >
                                                            <i class="fas fa-plus-circle" >&nbsp; Agregar</i>
                                                        </button>
                                                    ...
                                                    <div class="table-responsive ">
                                                        ...
                                                        <table class="table table-hover" id="tablitas1">
                                                            <thead>
                                                                <tr>
                                                                    <th>Detalle</th>
                                                                    <th>Producto </th>
